...
默认配置 `addons/sourcemod/configs/bot_stuff.cfg` 中 `randomizer=1`。每次换图后服务器会从阵容表随机选择两支不同的职业队，分别建立 CT 和 T 阵容；不是从全部选手中逐个随机拼队。

每方始终以 5 个参赛席位计算：没有真人时加入完整 5 人阵容；真人进入某方后只移除该方所需数量的 BOT；真人退出后只补回缺少的该阵容选手。这个增量处理不会为了调整人数而重建仍在场的整队 BOT。

中场以及加时半场真正换边时，职业队阵容应随 BOT 一起换到对侧，现有 BOT 不应被全队踢出再重新加入。进入下一次加时但未实际换边时则保持阵容不变；两方人数和 Logo 随实际阵营更新。`sm_bot_teams_status` 显示的是当前 CT/T 归属，不是开图时的原始归属。
...
- 可用 `sm_bot_teams_status` 查看当前随机开关、两方选中的阵容以及真人/BOT 人数。
- 要固定阵容，可将 `bot_stuff.cfg` 的 `randomizer` 改为 `0`，并在 `ct_team`、`t_team` 填写下表中的完整队名；也可以在运行时使用上述 `team` 指令单独更换一方。
