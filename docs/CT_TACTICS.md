...
- 持包保护：初始目标 0 固定优先分配给持有 C4 的 BOT，避免持包者被随机派到牵制位。
- 掉包回收：一名 BOT 直接取包，另派最多两名 BOT 占据周围接应位置。
- 下包后：存活 T 分散到炸弹周围 NAV 掩体，并以 CT 出生方向作为主要回防方向，由共用架点规划器按中央、左右 35°、左右 70°分配扇区；每个扇区再对附近角度做视线检测和 NAV 投影。AWP/狙击枪优先约 950 单位，步枪约 750 单位，SMG/霰弹枪约 500 单位。到达目标 128 单位内停止闲逛，偏离超过 256 单位才重新寻路；发现敌人、开火或受伤后立即解除个人站位和固定朝向，恢复原战斗 AI。
