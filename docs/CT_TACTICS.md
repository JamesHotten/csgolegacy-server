...
- 中期重新组织：只有同一包点附近 10 秒内目击至少 2 名不同 T，标准阵型才会再调一名不同的 CT 支援，通常保留另一点的最后一人。若目击至少 4 名不同 T，或至少 3 名且该点有 CT 被 T 击杀，则认定为强攻，允许最后的弱侧守点者回防；正在交火的 BOT 不会被强制改道。每个包点每回合最多执行一次普通重组，全转全队每回合最多一次，不会因同一个敌人反复露头而来回跑。中路目击不算包点强攻。原来的 70 秒节点现在只复查阵型，不会无条件清空防守命令。
- 确认看到持包者：无论现有阵型或普通报点冷却，所有未在交火的存活 CT BOT 立即向最后确认的持包位置回防；目击者继续交火。若 8 秒后再次目击，且持包者已移动至少 500 单位，才更新目标，避免频繁折返。掉包和下包后继续走既有守包、拆包回防流程；只是看到普通 T 不会触发全员回防。
...
在聊天栏输入 `!sidewin`，或在服务器控制台输入 `sm_sidewin`，可查看本次地图运行、当前地图累计和全服累计的 CT/T 胜局与胜率。胜率分母只包含有 CT 或 T 获胜方的正式回合；热身、平局和重复的回合结束事件不计。换边后的胜利按获胜阵营计算，不按职业队名计算。

统计从安装 `000_bot_side_win_stats.smx` 后开始，不倒算 RankMe 历史数据。累计结果保存在 `csgo/addons/sourcemod/data/bot_side_win_stats.txt`，跨换图、重启保留；迁移服务器时请连同该文件备份。本次地图运行数据会在换图或重启时清零。该插件不修改经济、计分板或原有玩家数据库。
...
- 中期转点：当前目标包点附近 10 秒内目击至少 2 名不同 CT、正式进攻已持续至少 12 秒、包匪离目标仍超过 800 单位、至少 3 名 T 存活且回合还剩至少 55 秒时，最多转向另一包点一次。包掉落、开始下包或时间紧迫时不转；转点后持包者和安静的队友集中到新点，正在交火者继续由 BetterBots 战斗，旧包点的开局预录道具跑位会取消。此判断依赖 BOT 的真实可见敌人信号和包点/NAV 坐标，不是固定时间随机换点。
...
sm_bot_ct_mid_reorganize_enable 1
...
15. 让 T 在目标点外连续确认两名不同 CT，且回合剩余超过 55 秒，观察最多一次转点、包匪随主攻走；接近包点、包掉落或时间不足时不得转点。
16. 让 CT 在同点短时间看到两名不同 T，确认普通补防仍留弱侧守点；再模拟 4 人强攻，或 3 人强攻加该点 CT 阵亡，确认允许全转。中路单人反复露头不应触发全转；下包后依旧走原有拆弹回防流程。
