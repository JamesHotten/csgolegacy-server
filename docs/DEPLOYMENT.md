...
要切换为 CS2 MR12 赛制，在服务器控制台执行 `exec mr12`。该配置同时设置 24 回合、先到 13 分结束、MR3 加时、加时 `$10,000` 初始经济，并启用完整 CS2 经济。执行 `exec mr15` 恢复 30 回合、先到 16 分、无 Premier 加时和完整 CS:GO Legacy 经济；切换后应使用 `mp_restartgame 1` 开始新比赛。
