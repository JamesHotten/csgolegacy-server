...
public void OnPluginStart()
{
	...
	g_hRandomLogos = CreateConVar("teamlogo_randomlogos", "0", "Enables independent random team logos on map load; keep disabled when BetterBots selects rosters");
...
public void OnConvarChanged(Handle cvar, const char[] oldVal, const char[] newVal)
{
	...
	else if (cvar == g_hTeamLogo1)
	{
		...
		AddTeamLogoToDownloadTable(newVal);
...
public void OnConfigsExecuted()
{
	...
	AddCurrentTeamLogosToDownloadTable();
}

stock void DiscoverTeamLogos()
{
	...
	while (ReadDirEntry(dir, filename, sizeof(filename), type))
	{
		...
		if (g_bRandomLogos)
		{
			...
			if (strlen(filename) <= 5)
				...
				LogError("Skipping team logo ID longer than 5 characters: '%s'", filename);
		}
	}
	CloseHandle(dir);
...
stock void AddCurrentTeamLogosToDownloadTable()
{
	char logo1[MAX_TEAMNAME_LENGTH];
	char logo2[MAX_TEAMNAME_LENGTH];
	GetConVarString(g_hTeamLogo1, logo1, sizeof(logo1));
	GetConVarString(g_hTeamLogo2, logo2, sizeof(logo2));

	AddTeamLogoToDownloadTable(logo1);
	if (!StrEqual(logo1, logo2))
		AddTeamLogoToDownloadTable(logo2);
}

stock void AddTeamLogoToDownloadTable(const char[] logo)
{
	if (logo[0] == '\0')
		return;
	...
	if (strlen(logo) > 5)
	{
		LogError("Cannot register team logo ID longer than 5 characters: '%s'", logo);
		return;
	}

	char path[PLATFORM_MAX_PATH];
	Format(path, sizeof(path), "materials/panorama/images/tournaments/teams/%s.svg", logo);
	if (FileExists(path, true))
	{
		AddFileToDownloadsTable(path);
		PrintToServer("[SM] Registered current team logo: %s", path);
		return;
	}

	Format(path, sizeof(path), "materials/panorama/images/tournaments/teams/%s.png", logo);
	if (FileExists(path, true))
	{
		AddFileToDownloadsTable(path);
		PrintToServer("[SM] Registered current team logo: %s", path);
	...
	Format(path, sizeof(path), "resource/flash/econ/tournaments/teams/%s.png", logo);
	if (FileExists(path, true))
	{
		AddFileToDownloadsTable(path);
		PrintToServer("[SM] Registered current team logo: %s", path);
	...
	LogError("Current team logo file not found for ID '%s'", logo);
}
