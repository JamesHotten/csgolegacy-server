...
- `sv_lan 1`
- `-insecure`
- 默认执行 `mr15.cfg`：MR15、无 Premier 加时、完整 CS:GO Legacy 经济
- `exec mr12`：MR12、MR3 加时、完整 CS2 经济；`exec mr15` 可完整恢复
...
LAN 真人以“IP＋昵称”生成稳定的合成 Steam2 身份。RankMe、计分板段位、武器皮肤、贴纸、手套、探员、喷漆、音乐盒和相关偏好均按此身份独立保存；正常 Steam 玩家不受影响。在同一张地图中断线重连还会恢复击杀、死亡、助攻、MVP、贡献分和官方竞技规则下的离线经济，计分板身份会在连接期间低频自检并修复。同 IP、同昵称的同时在线玩家会自动分配独立会话。现金账户在离线期间继续按实际回合结果、胜负原因、队伍连败档位和当前 `cash_team_*` 设置累计团队奖励；存活时断线的当前回合不发回合奖励，个人行为奖励不补发。重连原阵营后恢复一次并继续由游戏原生经济系统结算；换边或换图使用游戏正常的重置经济。换图后局内缓存清空，但数据库中的 MOD 配置保留。模式 `0/1` 会作为统一事务切换下包被拆补助、差异击杀奖励、差异购买价格、退款能力及 CS2 CT 淘汰补助；失败时回滚全部经济参数。成功切换只清理尚未恢复的局内经济和退款账本，不会改动战绩、身份或 MOD 数据。CS:GO 客户端购买菜单仍可能显示本地旧价格，但服务器实际扣款、提示和退款均使用当前模式价格。
